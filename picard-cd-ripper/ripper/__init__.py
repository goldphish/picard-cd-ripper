...
from picard.util import partial, encode_filename, sanitize_filename

from ui import Ui_CDRipperUI
...
class RipCDDialog(QtGui.QDialog):

  def __init__(self, parent):
    QtGui.QDialog.__init__(self, parent)
    self.ui = Ui_CDRipperUI()
...
class CDRipper(QtCore.QObject):
  ...
  def errorHandler(self, error):
    ...
    self.log.debug(msg)
    self.widget.ui.rip_output.appendPlainText(QtCore.QString(msg))
    ...
    for process in self._processes:
      if self._ripping:
        self.widget.ui.rip_output.appendPlainText(
            QtCore.QString(process.readAll()))
      else:
